...
const Cart = ({ itemCount = 0, totalAmount = 0 }) => {
    ...
    if (totalAmount >= 1000000000) {
        formattedAmount = Math.round(totalAmount / 1000000000);
        symbol = 'B';
    } else if (totalAmount >= 1000000) {
        formattedAmount = Math.round(totalAmount / 1000000);
        symbol = 'M';
    } else if (totalAmount >= 1000) {
        formattedAmount = Math.round(totalAmount / 1000);
        symbol = 'K';
    }

    const displayedAmount = formattedAmount + symbol;
        ...
        <div className={styles.cart}>
            <Link href={'/basket'}>
                ...
                <div className={styles["cart__itemCount"]}>
                    {itemCount > 9 ? <span>9+</span> : <span>{itemCount}</span>}
                </div>
            </Link>
            <div className={styles["cart__info"]}>
                <span className={styles["cart__info__text"]}>{t('cart')}</span>
                <span className={styles["cart__info__amount"]}>{displayedAmount} ₽</span>
            </div>
        </div>
    );
};
